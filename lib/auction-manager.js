...
export class AuctionManager {
  ...
  _handleComment(comment) {
    ...
    this.broadcast({ type: 'new-comment', comment });

    // Parse bid from comment text
    const parsed = parseBid(comment.text);
    ...
    const bid = {
      ...
      verified: comment.verified !== false, // propagate unverified flag from FB
   ...
   */
  async connect(platform, identifier) {
    try {
      switch (platform) {
        case 'youtube': {
        ...
        case 'facebook': {
          ...
          await this._facebook.connect(identifier);
          break;
        }
        case 'instagram': {
