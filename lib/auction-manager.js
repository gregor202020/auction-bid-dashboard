...
export class AuctionManager {
  /**
   * @param {{ youtubeApiKey?: string, youtubeApiKeys?: string[]|string, metaAccessToken?: string }} credentials
    ...
    try {
      switch (platform) {
        case 'youtube': {
          ...
          const youtubeKeys = this.credentials.youtubeApiKeys?.length
            ? this.credentials.youtubeApiKeys
        ...
        case 'facebook': {
          ...
          const preflight = await this.fbPreflight(identifier, originalIdentifier);
          if (!preflight.ok) {
            throw new Error(preflight.error || 'Facebook preflight failed. Login to Facebook and retry.');
          }

          // Start browser scraper for username enrichment with a verified session.
  ...
  async fbLogin() {
    if (!this._fbScraper) this._fbScraper = new FbBrowserScraper();
    const result = await this._fbScraper.startLoginFlow();
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  }
  ...
  async fbLoginClick(x, y) {
    ...
    const result = await this._fbScraper.loginClick(x, y);
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  ...
  async fbLoginType(text) {
    if (!this._fbScraper) return { error: 'No scraper' };
    const result = await this._fbScraper.loginType(text);
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  ...
  async fbLoginKeyPress(key) {
    if (!this._fbScraper) return { error: 'No scraper' };
    const result = await this._fbScraper.loginKeyPress(key);
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  ...
  async fbLoginWheel(deltaX, deltaY) {
    if (!this._fbScraper) return { error: 'No scraper' };
    const result = await this._fbScraper.loginWheel(deltaX, deltaY);
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  ...
  async fbLoginScreenshot() {
    if (!this._fbScraper) return { error: 'No scraper' };
    const result = await this._fbScraper.loginScreenshot();
    if (result?.loggedIn) {
      await this._refreshFacebookScraper();
    }
    return result;
  ...
  async fbPreflight(identifier, originalIdentifier = identifier) {
    if (!this._fbScraper) this._fbScraper = new FbBrowserScraper();
    const target = originalIdentifier || identifier;
    if (!target) {
      return { ok: false, requiresLogin: false, error: 'Facebook video URL or video ID is required.' };
    }
    return await this._fbScraper.preflightSession(target);
  }

  async _refreshFacebookScraper() {
    if (!this._facebook?.isConnected()) return;
    await this._facebook.refreshScraper();
  }
