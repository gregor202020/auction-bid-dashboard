...
function extractFacebookVideoId(value) {
  if (!value) return null;

  let match = String(value).match(/videos\/(\d+)/);
  if (match) return match[1];

  match = String(value).match(/[?&]v=(\d+)/);
  if (match) return match[1];

  match = String(value).match(/\b(\d{8,})\b/);
  if (match) return match[1];
...
export class FacebookLivePoller {
   ...
   */
  constructor(accessToken, onComment, scraper, onStateChange = null) {
  ...
  async _resolveTarget(identifier, originalIdentifier) {
    const candidates = [originalIdentifier, identifier].filter(Boolean);
    let videoId = candidates.map(extractFacebookVideoId).find(Boolean) || null;
    let pageUrl = candidates.find(isHttpUrl) || null;

    if (!videoId && pageUrl) {
      try {
        const res = await fetch(pageUrl, { redirect: 'follow' });
        videoId = extractFacebookVideoId(res.url);
        if (!videoId) {
          const html = await res.text();
          videoId = extractFacebookVideoId(html);
        }
      } catch (err) {
        console.warn('[Facebook] Failed to resolve video URL:', err.message);
      }
    }

    if (!videoId) {
      throw new Error(`Could not resolve a Facebook video ID from "${identifier}"`);
    }

    if (!pageUrl) {
      pageUrl = `https://www.facebook.com/watch/live/?v=${videoId}`;
    }
   ...
   */
  async connect(identifier, originalIdentifier = identifier) {
    this._running = false;
    this._videoId = null;
    this._seenIds.clear();
    this._afterCursor = null;
    ...
    const { videoId, pageUrl } = await this._resolveTarget(identifier, originalIdentifier);
    ...
    if (this._scraper) {
      try {
        const scraperStarted = await this._scraper.connect(pageUrl);
        if (scraperStarted) {
  ...
  async refreshScraper() {
    if (!this._running || !this._scraper || !this._videoId) return false;

    try {
      const scraperStarted = await this._scraper.connect(this._pageUrl || this._videoId);
      if (scraperStarted) {
        console.log('[Facebook] Browser scraper refreshed with saved session');
      }
      return scraperStarted;
    } catch (err) {
      console.warn('[Facebook] Failed to refresh browser scraper:', err.message);
      return false;
    }
  }
      ...
      try {
        ...
        if (probeRes.ok) {
          ...
          if (probeData.data?.length && !probeData.data[0].from) {
            console.warn('[Facebook] API is omitting commenter names for this live stream � scraper enrichment will be used.');
  ...
  async _fetchPage(afterCursor, depth = 0) {
    ...
    if (!res.ok) {
      ...
      if (/after|cursor/i.test(errorMsg)) {
        console.warn('[Facebook] Invalid pagination cursor, resetting and retrying...');
        this._afterCursor = null;
        return;
      }
      if (res.status >= 500) {
        console.warn('[Facebook] Transient poll error:', errorMsg);
        return;
      }
    ...
    for (const comment of comments) {
      ...
      if (!username && this._scraper) {
        const scraperMatch = this._scraper.lookupUsername(comment.message, createdTime || Date.now());
        if (scraperMatch) {
