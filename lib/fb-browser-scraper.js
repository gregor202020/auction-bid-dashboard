...
export class FbBrowserScraper {
  ...
  async _checkLoginSuccess() {
    ...
    try {
      ...
      const loginPromptVisible = await this._isLoginPromptVisible(this._loginPage);

      const looksLoggedIn = hasSessionCookies &&
        isFacebookDomainUrl(url) &&
        !hasLoginLikeUrl(url) &&
        !loginPromptVisible;
      if (looksLoggedIn) {
  ...
  _buildVideoUrl(pageTarget) {
    return /^https?:\/\//i.test(String(pageTarget || ''))
      ? String(pageTarget)
      : `https://www.facebook.com/watch/live/?v=${pageTarget}`;
  }

  /**
   * Validate the saved Facebook session against the target live URL.
   * Does not keep a scraping session open.
   */
  async preflightSession(pageTarget) {
    const puppeteer = await this._loadPuppeteer();
    if (!puppeteer) {
      return { ok: false, requiresLogin: true, error: 'Puppeteer not installed on server.' };
    }

    if (this._loginActive) {
      return { ok: false, requiresLogin: true, error: 'Facebook login is already in progress. Complete or cancel it first.' };
    }

    if (this._running && this._page) {
      const currentUrl = this._page.url();
      const loginPromptVisible = await this._isLoginPromptVisible(this._page);
      if (!hasLoginLikeUrl(currentUrl) && !loginPromptVisible) {
        return { ok: true, checkedUrl: currentUrl, alreadyConnected: true };
      }
    }
    ...
    if (cookies.length === 0) {
      clearSavedCookies();
      return { ok: false, requiresLogin: true, error: 'No saved Facebook session. Click "Login to Facebook" first.' };
    }

    const videoUrl = this._buildVideoUrl(pageTarget);
    let browser = null;
    let page = null;

    try {
      browser = await puppeteer.launch(this._getLaunchOptions());
      ...
      const loginPromptVisible = await this._isLoginPromptVisible(page);
      if (hasLoginLikeUrl(currentUrl) || loginPromptVisible) {
        clearSavedCookies();
        return {
          ok: false,
          requiresLogin: true,
          error: 'Facebook session expired or checkpointed. Re-login in Settings before connecting Facebook.',
        };
      }

      return { ok: true, checkedUrl: currentUrl };
    } catch (err) {
      return {
        ok: false,
        requiresLogin: true,
        error: `Facebook preflight failed: ${err.message}`,
      };
    } finally {
      if (page) {
    ...
    try {
      const comments = await this._page.evaluate(() => {
        ...
        const addResult = (username, text) => {
          ...
          if (['like', 'comment', 'share', 'follow'].includes(lowered)) return;

          const key = `${cleanName}\u0000${cleanText}`;
          if (!results.has(key)) {
            results.set(key, { username: cleanName, text: cleanText });
          }
        };

        // Strategy 1: article-based candidates
        for (const article of document.querySelectorAll('[role="article"]')) {
          const nameCandidates = article.querySelectorAll('a[role="link"] span, a.x1i10hfl span, h3 span');
          const textCandidates = article.querySelectorAll('[data-ad-comet-preview="message"] [dir="auto"], [dir="auto"]');
          const username = nameCandidates[0]?.textContent?.trim();
          const text = [...textCandidates]
            .map((node) => node.textContent?.trim())
            .find((value) => value && value.length > 0 && value !== username);
          addResult(username, text);
        }

        // Strategy 2: live comment containers
        for (const el of document.querySelectorAll('[data-testid="UFI2Comment/root_depth_0"], [role="listitem"], div[aria-label*="comment" i]')) {
          const username = el.querySelector('a span, [data-testid="UFI2CommentActorName"] span, h3 span')?.textContent?.trim();
          const text = el.querySelector('[data-testid="UFI2CommentBody"] span, [data-ad-preview="message"] span, [dir="auto"]')?.textContent?.trim();
          addResult(username, text);
        }

        // Strategy 3: compact span pairs
        for (const item of document.querySelectorAll('ul > li, [role="listitem"]')) {
          const spans = [...item.querySelectorAll('span')]
            .map((span) => span.textContent?.trim())
            .filter(Boolean);
          if (spans.length >= 2) {
            addResult(spans[0], spans[1]);
          }
        }
  ...
  lookupUsername(commentText, referenceTime = null) {
    ...
    if (referenceTime !== null) {
      const nearest = this._getNearestUsernameForKey(key, referenceTime);
    ...
    if (stripped !== key) {
      ...
      if (referenceTime !== null) {
        const nearestStripped = this._getNearestUsernameForKey(stripped, referenceTime);
        if (nearestStripped) return nearestStripped;
      }
    }
  ...
  _recordCommentMatch(username, text) {
    ...
    if (!entry) {
      entry = { usernames: new Map(), events: [], lastSeen: now };
      this._commentMap.set(key, entry);
    }

    entry.usernames.set(username, now);
    entry.events.push({ username, seenAt: now });
    if (entry.events.length > 40) {
      entry.events.splice(0, entry.events.length - 40);
    }
    entry.lastSeen = now;
  ...
  _getNearestUsernameForKey(key, referenceTime) {
    const entry = this._commentMap.get(key);
    ...
    const candidates = entry.events
      .filter((event) => Math.abs(event.seenAt - ref) <= windowMs)
      .sort((a, b) => {
        const aDelta = Math.abs(a.seenAt - ref);
        const bDelta = Math.abs(b.seenAt - ref);
    ...
    const competing = candidates.find((event) =>
      event.username !== best.username &&
      Math.abs(Math.abs(event.seenAt - ref) - Math.abs(best.seenAt - ref)) <= 1500
    );
  ...
  async _isLoginPromptVisible(page) {
    ...
    try {
      return await page.evaluate(() => {
        ...
        const loginAction = [...document.querySelectorAll('button, input[type="submit"], div[role="button"]')]
          .map((node) => (node.innerText || node.value || '').trim().toLowerCase())
          .some((text) => text === 'log in' || text === 'login');
        if (loginAction && (email || pass)) return true;

        const body = (document.body?.innerText || '').toLowerCase();
        return body.includes('see more on facebook') && body.includes('log in');
      });
    } catch (_) {
      return false;
    }
