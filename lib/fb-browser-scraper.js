...
export class FbBrowserScraper {
  ...
  async loginKeyPress(key) {
    ...
    try {
      ...
      return await this._buildInteractionResult({ expectNavigation: key === 'Enter' || key === 'Tab' });
    } catch (err) {
      return { error: err.message };
    }
  }
  ...
  async _waitForLoginPageSettled(options = {}) {
    ...
    const { expectNavigation = true } = options;
    if (expectNavigation) {
      try {
        await Promise.race([
          this._loginPage.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 4000 }).catch(() => null),
