...
export class YouTubeLivePoller {
  /**
   * @param {string|string[]} apiKey - YouTube Data API key (or CSV/list for failover)
  ...
  constructor(apiKey, onComment, onStateChange = null) {
    this.apiKeys = Array.isArray(apiKey)
      ? apiKey.map((value) => String(value || '').trim()).filter(Boolean)
      : String(apiKey || '')
        .split(',')
  ...
  _isQuotaError(status, message, reason = '') {
    if (status !== 403) return false;
    const detail = `${message || ''} ${reason || ''}`.toLowerCase();
    return detail.includes('quota');
  }

  _rotateApiKey(context = '') {
    if (!this.apiKeys.length || this._keyIndex >= this.apiKeys.length - 1) {
      return false;
    }
    this._keyIndex += 1;
    this.apiKey = this.apiKeys[this._keyIndex];
    console.warn(`[YouTube] Switching to fallback API key ${this._keyIndex + 1}/${this.apiKeys.length}${context ? ` (${context})` : ''}`);
    return true;
  }
  ...
  async _getLiveChatId(videoId) {
    while (true) {
      ...
      if (!res.ok) {
        ...
        if (this._isQuotaError(res.status, message, reason) && this._rotateApiKey('quota exceeded during connect')) {
          continue;
        }
        if (this._isQuotaError(res.status, message, reason)) {
          throw new Error('YouTube API quota exceeded for all configured API keys.');
        }
  ...
  async _poll() {
    ...
    let pollingInterval = 5000; // default fallback
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (code === 400 || /page.?token/i.test(msg)) {
          ...
          if (this._isQuotaError(res.status, msg, reason) && this._rotateApiKey('quota exceeded while polling')) {
        ...
        // Respect API's recommended interval and avoid aggressive polling to conserve quota.
        pollingInterval = Math.max(data.pollingIntervalMillis || 5000, 5000);
