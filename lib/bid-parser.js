...
const BID_KEYWORD_RE = /\b(?:bid|bidding|offer|going|raise|mine|take|want|pay|ill\s+(?:go|do|take|pay)|i'?ll\s+(?:go|do|take|pay)|me\s+for|mine\s+for|yes\s+for)\b/i;

// Currency patterns
const CURRENCY_PREFIX_RE = /(?:\$|AUD\s*\$?|A\$|NZD\s*\$?)/i;
const CURRENCY_WORD_RE = /\b(?:dollars?|bucks?|aud|usd|nzd)\b/i;
...
const STRIPPABLE_AMOUNT_WORD_RE = /\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred|thousand|and|k|dollars?|bucks?|aud|usd|nzd)\b/gi;
const NEGATIVE_CONTEXT_RE = /\b(?:percent(?:age)?|stars?|likes?|viewers?|followers?|subs?|subscribers?|minutes?|mins?|hours?|hrs?|days?|weeks?|months?|years?|kg|grams?|lbs?|pounds?|cm|mm|inches?|feet|foot|ft)\b/i;
const ALLOWED_AMOUNT_FILLER_WORDS = new Set([
  'i', 'ill', 'll', 'bid', 'bidding', 'offer', 'going', 'raise', 'mine',
  'take', 'want', 'pay', 'go', 'do', 'for', 'it', 'me', 'yes', 'please',
  'pls', 'now', 'ok', 'okay'
]);
...
export function parseBid(text) {
  ...
  let confidence = 'low';
  let amount = null;
  const hasBidContext = BID_KEYWORD_RE.test(trimmed);
  const hasCurrencyContext = CURRENCY_PREFIX_RE.test(trimmed) || CURRENCY_WORD_RE.test(trimmed);
  const standaloneAmountComment = isStandaloneAmountComment(trimmed);

  const acceptCandidate = (candidate, defaultConfidence = 'medium') => {
    if (!isAcceptableBidCandidate(trimmed, candidate, { hasBidContext, hasCurrencyContext, standaloneAmountComment })) {
      return false;
    }
    amount = candidate;
    confidence = (hasBidContext || hasCurrencyContext) ? 'high' : defaultConfidence;
    return true;
  };
  ...
  if (kMatch) {
    acceptCandidate(parseFloat(kMatch[1]) * 1000);
  }
  ...
  if (amount === null) {
    ...
    if (hundredMatch) {
      acceptCandidate(parseInt(hundredMatch[1], 10) * 100);
    }
  }
  ...
  if (amount === null && NUMBER_WORD_RE.test(trimmed)) {
    ...
    if (wordAmount !== null) acceptCandidate(wordAmount);
  }
  ...
  if (amount === null) {
    ...
    if (currencyMatch) {
      acceptCandidate(parseAmount(currencyMatch[1]), 'high');
    }
  }
  ...
  if (amount === null) {
    const keywordMatch = trimmed.match(
      /\b(?:bid|bidding|offer|going|raise|mine|take|want|pay|go)\s*(?:it\s+)?(?:for\s+)?(?:\$|AUD\s*\$?|A\$)?\s*((?:\d{1,3}(?:,\d{3})+|\d{1,7})(?:\.\d{1,2})?)/i
    );
    if (keywordMatch) {
      acceptCandidate(parseAmount(keywordMatch[1]), 'high');
    }
  }
  ...
  if (amount === null) {
    ...
    if (suffixMatch) {
      acceptCandidate(parseAmount(suffixMatch[1]), 'high');
    }
  }
  ...
  if (amount === null) {
    ...
    if (plainMatch) {
      acceptCandidate(parseAmount(plainMatch[1]), 'medium');
    }
  }

  // Strategy 5: Number appears in a short comment (< 80 chars)
  // Only allow this fallback when the comment still looks like a bid.
  if (amount === null && trimmed.length < 80 && (hasBidContext || hasCurrencyContext || standaloneAmountComment)) {
    const looseMatch = trimmed.match(
      /((?:\d{1,3}(?:,\d{3})+|\d{1,7})(?:\.\d{1,2})?)/
    );
    if (looseMatch) {
      const candidate = parseAmount(looseMatch[1]);
      if (candidate !== null) acceptCandidate(candidate, candidate >= 10 ? 'medium' : 'low');
    }
  }
...
function isStandaloneAmountComment(text) {
  const stripped = text.toLowerCase()
    .replace(/(?:\$|aud\s*\$?|a\$|nzd\s*\$?)/gi, ' ')
    .replace(/(?:\d{1,3}(?:,\d{3})+|\d{1,7})(?:\.\d{1,2})?/g, ' ')
    .replace(STRIPPABLE_AMOUNT_WORD_RE, ' ')
    .replace(/[^a-z\s]/g, ' ');

  const tokens = stripped.match(/[a-z]+/g) || [];
  return tokens.every((token) => ALLOWED_AMOUNT_FILLER_WORDS.has(token));
}

function isAcceptableBidCandidate(text, candidate, { hasBidContext, hasCurrencyContext, standaloneAmountComment }) {
  if (candidate === null || !isFinite(candidate)) return false;
  if (candidate <= 0 || candidate > 999999) return false;
  if (candidate < 5 && !hasBidContext && !hasCurrencyContext) return false;
  if (candidate >= YEAR_RANGE.min && candidate <= YEAR_RANGE.max && !hasBidContext && !hasCurrencyContext) {
    return false;
  }
  if (NEGATIVE_CONTEXT_RE.test(text) && !hasBidContext && !hasCurrencyContext) {
    return false;
  }
  return hasBidContext || hasCurrencyContext || standaloneAmountComment;
}
